  ...
  const handle = async (e) => {
    ...
    try {
      ...
      if (form.email && form.password) {
        const mockUser = { name: 'مستخدم تجريبي', email: form.email, role: form.email.includes('admin') ? 'admin' : 'user' }
    ...
    <div className="min-h-screen bg-gradient-to-br from-brand-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-4" dir="rtl">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">SecureCourse Pro</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">سجل دخولك للمتابعة</p>
        </div>
        ...
        <div className="card shadow-xl">
          ...
          <p className="text-center text-sm text-gray-500 dark:text-gray-400 mt-4">
            ليس لديك حساب؟{' '}
            <Link to="/register" className="text-brand-600 font-semibold hover:underline">إنشاء حساب</Link>
          </p>

          <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-xs text-blue-700 dark:text-blue-400">
            💡 للدخول كأدمن: استخدم أي إيميل يحتوي على "admin"
          </div>
        </div>
      </div>
    </div>
